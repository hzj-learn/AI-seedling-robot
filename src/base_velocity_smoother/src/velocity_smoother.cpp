...
#include <base_velocity_smoother/velocity_smoother.h>
...
void VelocitySmoother::reconfigCB(base_velocity_smoother::paramsConfig &config, uint32_t level)
{
...
void VelocitySmoother::velocityCB(const geometry_msgs::Twist::ConstPtr& msg)
{
  ...
  input_active = true;
//  ROS_INFO("cb_avg_time:=%lf",cb_avg_time);
...
void VelocitySmoother::spin()
{
  ...
  while (! shutdown_req && ros::ok())
  {
    if ((input_active == true) && (cb_avg_time > 0.0) &&
        ((ros::Time::now() - last_cb_time).toSec() > std::min(4.0*cb_avg_time, 0.5))) //3.0
    {
...
bool VelocitySmoother::init(ros::NodeHandle& nh)
{
  ...
  dynamic_reconfigure_server = new dynamic_reconfigure::Server<base_velocity_smoother::paramsConfig>(nh);
