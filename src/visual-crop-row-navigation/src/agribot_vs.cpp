...
void judge_circle(Point2f& pt1, float& rd1, Point2f& pt2, float& rd2, int& count_c, int& count_d)
	{
	    double dis = hypot(pt1.x-pt2.x, pt1.y-pt2.y);  // 圆心距离
	    double rs = fabs(rd1 - rd2);  //两圆半径差

	    if (rs == dis || rs > dis)  // 内切、内含
		{
		    if (rd1 >= rd2)   // rd1 包含 rd2
			{
			    ++count_c;
			}
		    else
  ...
  vector<vector<Point>> AgribotVS::CropRowFeatures(camera& camera) {
    ...
    int s2 = 3;
    int s3 = 3;   
    Mat structure_element = getStructuringElement(MORPH_RECT, Size(s1,s1)); //设置膨胀/腐蚀的核为矩形，大小为3*3
    Mat structure_element2 = getStructuringElement(MORPH_RECT, Size(s2,s2));
    Mat structure_element3 = getStructuringElement(MORPH_RECT, Size(s3,s3));
    vector<vector<Point>> contours;
    vector<Vec4i> hierarchy;
    RNG rng(12345);

    // 形态学处理，膨胀与腐蚀
    dilate(hueMask, hueMask, structure_element); //膨胀
    dilate(hueMask, hueMask, structure_element3); 
    erode(hueMask, hueMask, structure_element2); //腐蚀
    dilate(hueMask, hueMask, structure_element3);

    //cv::imshow("阈值图",hueMask );
  ...
  vector<Point2f> AgribotVS::getContureCenters(Mat& img, vector<vector<Point>>& contours){
    ...
    // // find enclosing Polygon which fits arround the contures 
    // for (size_t i = 0; i < contours.size(); i++) {
    //   approxPolyDP(Mat(contours[i]), contours_poly[i], 2, true);
    //   minEnclosingCircle((Mat)contours_poly[i], center[i], radius[i]);
    //   cv::circle(img, Point(center[i].x, center[i].y),3, Scalar(255, 0, 0),CV_FILLED, 8,0);
    // }
    // return center;
    ...
    vector<Point2f> countours_center1;
    vector<Point2f> countours_center;
    vector<float> countours_radius;

    int size_contours = contours.size();
    for(int i = 0; i < size_contours;i++)
      {
        float radius =0;
        minEnclosingCircle(contours[i], center[i], radius);
        center1[i].x=center[i].x;
        center1[i].y=center[i].y;
        radius1[i]=radius;
        countours_center1.push_back(center1[i]);
      }
    ...
    for(int i = 0; i < size_contours; i++)
      {
        ...
        for(int j = 0; j < size_contours;j++)
        {	             
          judge_circle(center1[i], radius1[i], center1[j], radius1[j], count_a, count_b);  //判断两圆关系

        }
    
        if (count_a > 0 && count_b == 0)
        {   
          if(radius1[i] >= 30)     // 筛选符合半径大小的圆
          {
              countours_center.push_back(center1[i]);
              countours_radius.push_back(radius1[i]);
          }
        }
      }

    // for (int i = 0; i < countours_center.size(); i++)
    // {
    //   cout<<"第 "<<i<<"个圆的半径大小为"<<countours_radius[i]<<endl;
    // }
     
    return countours_center;
  ...
  vector<Point2f> AgribotVS::filterContures(Mat& img, vector<vector<Point>>& contours){
    ...
    for (size_t i = 0; i < contours.size(); i++) {
      ...
      if (center[i].x >= (img.cols/2) - center_min_off && 
        ...
        cout<<"center_min_off: "<<center_min_off<<endl;
  ...
  vector<Vec4i> AgribotVS::FitLineOnContures(Mat& img, vector<Point2f>& ContourCenters){
    ...
    if(ContourCenters.size() > 0 ){
      ...
      cv::line(img, Point(R_out(0,0), R_out(0,1)),Point(R_out(1,0), R_out(1,1)), Scalar(0, 0, 255), 1, CV_AA);   // 绘制所拟合的直线，红色
  ...
  void AgribotVS::Controller(camera& I_primary, camera& I_secondary){
    ...
    // if(abs(v) < 0.05 ||  (I_primary.nh_points.size() < 5  && I_secondary.nh_points.size() <5)){
      if(abs(v) < 0.05 ||  (I_primary.nh_points.size() < 2  && I_secondary.nh_points.size() <2)){
    ...
    // ROS_INFO_STREAM("VSMsg.err_x: "<<VSMsg.err_x);
    // ROS_INFO_STREAM("VSMsg.err_theta: "<<VSMsg.err_theta);
  }
  void AgribotVS::compute_feature_point(camera& I){
    ...
    I.nh_ex = Compute_nh_ex(I);   //找到检测框中的y值最小的点和最大的点， nh_ex (minPoint(x1, y1),maxPoint(x2, y2))
    I.nh.Xc = (I.nh_ex[0].x + I.nh_ex[1].x)/2; // 取最大点和最小点的平均值作为检测框的中心点。
    ...
    // cv::circle(I.image, Point(Q.x, Q.y),8, Scalar(0,255,255),CV_FILLED, 12,0);  //黄点bgr(0, 255, 255)

    if(drive_forward){
      cv::circle(I.image, Point(Q.x, Q.y),8, Scalar(0,0,255),CV_FILLED, 12,0);    // 红点bgr, 终点为红点
      cv::circle(I.image, Point(P.x, P.y),8, Scalar(0,255,255),CV_FILLED, 12,0);  // 黄点bgr(0, 255, 255)
      // cv::arrowedLine(I.image, Q, P, Scalar(0,255,255),5);  //绘制黄色将头线代表所拟合的直线及方向
    }else
    {
      cv::circle(I.image, Point(P.x, P.y),8, Scalar(0,0,255),CV_FILLED, 12,0);    // 红点bgr, 终点为红点
      cv::circle(I.image, Point(Q.x, Q.y),8, Scalar(0,255,255),CV_FILLED, 12,0);  // 黄点bgr(0, 255, 255)
    }
    ...
    // compute F
    // 用来计算不同模式下箭头的位置和方向
    if(drive_forward){      // true：1,3 模式
    Point2f _F = camera2image(Q);
    // F << _F.x,
    //      _F.y,
    //      M_PI-Theta;
    // cout<<"前置角度："<<M_PI-Theta<<endl;
    F << _F.x,
         _F.y,
         0.1*(M_PI-Theta)+0.9*F_temp;
    F_temp = 0.5*sqrt(0.5*pow(F(2),2)+0.5*pow(F_temp,2));
  ...
  void AgribotVS::switching_controller(camera& I_primary, camera& I_secondary, unsigned int min_points){    
    ...
    is_in_neigbourhood(I_primary);   // 判断点是否在在检测框内，如果在，则把点加入I.nh_points（矩形框）内并作为检测到的作物的点。可视化时蓝点显示。
    ...
    if(I_primary.nh_points.size() < min_points){   // 此处传进来 min_points=min_points_switch=15
      minp_cnt++;   // 在命名空间中初始化时等于0
    }else{
      minp_cnt = 0;
    }

    ////// 判断是否要切换镜头camera_ID和模式mode。
    //// min_frame=15, coef=60, min_points=15, turning_mode:false， height=720， mode=1， drive_forward = true
    //判断主镜头内有没有目标点等其他条件
    ...
    if((I_primary.nh_points.size() == 0 && minp_cnt >=min_frame)|| 
      ...
      (avg_nh_points_y > (double)height-coef && avg_points_y > (double)height-coef*2)){
      minp_cnt=0;
      cout << "I_primary doesn't see anything !!!! ID: " << camera_ID << endl;     // camera_ID=1
      if(I_secondary.points.size() < min_points){    //主镜头内没有目标点，再判断次镜头内是否有目标点。如果次镜头判断检测分割的点数少于阈值15，则切换模式。
        cout << "turning_mode: " << turning_mode << endl;
          ...
          if(turning_mode){
            ...
            if(mode == 5)mode=1;
            cout << "TURNING..." << endl;
            // switch steering direction，driving_dir=1
      ...
      }else{     // 如果次镜头内有目标点或者目标点大于指定阈值，则执行这段，主要实现切换主次镜头。
          mode++;
          // 1->2 and 3->4
          cout << "SWITCHING CAMERAS" << endl;
          switch_cameras(camera_ID);    // 切换镜头camera_ID之后，相应的会将I_primary和I_secondary进行切换。
          initialize_neigbourhood(I_secondary);  // 重新初始化I.nh(检测框)位置。
          initialize_neigbourhood(I_primary);
          is_in_neigbourhood(I_primary);   
          turning_mode = true;
          drive_forward = false;
          steering_dir = -steering_dir;       // steering_dir=1
          cout << "turning mode ON" << endl;
          cout<<"进入2, 4模式："<<endl; 
      }
    }
    // 使用主镜头正常输出行进时的模式、镜头等参数。
    else{
        cout  << 
        "mode: " << mode << 
        ", cam: " <<  camera_ID << 
  ...
  void AgribotVS::initialize_neigbourhood(camera& I){
    ...
    I.nh.Xc = ex_Xc;   //ex_Xc=320
    I.nh.Yc = ex_Yc;   //ex_Yc=240
    I.nh.L = nh_L;    // nh_L=120
    I.nh.H = nh_H;    // nh_H=250
    I.nh.offset = nh_offset;   //nh_offset=200
    I.nh.shift = navigation_dir * I.nh.offset;   // navigation_dir=1
  ...
  void AgribotVS::shift_neighbourhood(camera& I, int shift_dir){
    ...
    int dX = shift_dir*I.nh.shift;   // I.nh.shift=200, 第一次传进来的 shift_dir=steering_dir= -1
    cout << "###########shift_neighbourhood############" << endl;
    // new neighbourhood to select row， dX=-200
    cout << I.nh.Xc  << " " << I.nh.Xc + dX  << " " << I.nh.shift << "  " << shift_dir << endl;
    I.nh.Xc = I.nh.Xc + dX;
  }
  // 判断点是否在在检测框内，如果在，则把点加入矩形框内并作为检测到的作物的点。可视化时蓝点显示。
  ...
  void AgribotVS::draw_neighbourhood(camera& I){
    ...
    int X = P.x - I.nh.L/2;
    int Y = P.y - I.nh.H/2;
    Rect RectangleToDraw3(X, Y, I.nh.L, I.nh.H);  // Rect(int a,int b,int c,int d)中 a,b为矩形的左上角坐标,c,d为矩形的长和宽
    // rectangle(I.image, RectangleToDraw3, Scalar(255, 204, 102), 3, 8, 0);
    rectangle(I.image, RectangleToDraw3, Scalar(0, 0, 255), 3, 8, 0);
  }
  //*************************************************************************************************
  void AgribotVS::draw_features(camera& I, Vector3f Feature, cv::Scalar color){
    ...
    arrowedLine(I.image, P_1, P_2, color,3);  //画带箭头的直线，color为bgr
  }

  // 找到检测框中的y值最小的点和最大的点放在 nh_ex中，nh_ex (minPoint(x1, y1),maxPoint(x2, y2))。
  vector<Point2f> AgribotVS::Compute_nh_ex(camera& I){
    vector<Point2f> nh_ex;
    nh_ex.resize(2);
    int tmp_min_y = height;   // height=400
    int tmp_max_y = 0;
    ...
    return nh_ex;  //找到检测框中的y值最小的点和最大的点
  }
